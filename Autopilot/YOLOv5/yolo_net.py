...
class YOLOv5_net:
    ...
    def __init__(self):
        ...
        self.out, self.source, self.weights, self.view_img, self.save_txt, self.img_size = \
            opt.save_dir, opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size
        self.use_webcam = self.source.isnumeric() or self.source.startswith(('rtsp://', 'rtmp://', 'http://')) or self.source.endswith('.txt')

        # Initialize
        set_logging()
        self.device = select_device(opt.device)
        if os.path.exists(self.out):  # output dir
            shutil.rmtree(self.out)  # delete dir
        os.makedirs(self.out)  # make new dir
    ...
    def detect(self, save_img=False):
        ...
        _ = self.model(img.half() if self.isHalf else img) if self.device.type != 'cpu' else None  # run once
        for path, img, im0s, vid_cap in self.dataset:
            ...
            for i, det in enumerate(pred):  # detections per image
                if self.use_webcam:  # batch_size >= 1
