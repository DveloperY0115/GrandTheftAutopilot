...
def get_test_loader(batch_size, data_root,dataset, distributed):
    ...
    if dataset == 'CULane':
        test_dataset = LaneTestDataset(data_root,os.path.join(data_root, 'list/tests.txt'),img_transform = img_transforms)
        cls_num_per_lane = 18
    elif dataset == 'Tusimple':
        test_dataset = LaneTestDataset(data_root,os.path.join(data_root, 'tests.txt'), img_transform = img_transforms)
        cls_num_per_lane = 56
